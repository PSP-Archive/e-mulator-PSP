...
//	fb1 = sceIoOpen(filename,O_RDONLY,0777);
    fb1 = HAL_fd_open((char*)filename,HAL_MODE_READ);
...
int ___fseek( FILE *stream, long offset, int origin )
{
    ...
    ulPtr = HAL_fd_seek(___zlibInFile.nFileId, offset, origin);
...
int ___fclose( FILE *stream )
{
    ...
    HAL_fd_close(___zlibInFile.nFileId);
	return 0;
}
...
size_t ___fread( void *buffer, size_t size, size_t count, FILE *stream )
{
    ...
    readSize = HAL_fd_read(___zlibInFile.nFileId, buffer, readSize);
