...
u8 internal_buffer[0x100];

/* Precalculated pixel table */
u16 pixel[PALETTE_SIZE];
...
//#ifdef ALIGN_DWORD
//
//static __inline__ u32 read_dword(void *address)
//{
//    if ((u32)address & 3)
//	{
...
//
//
//static __inline__ void write_dword(void *address, u32 data)
//{
//    if ((u32)address & 3)
//	{
//#ifdef LSB_FIRST
...
//}
//#else
//#define read_dword(address) *(u32 *)address
//#define write_dword(address,data) *(u32 *)address=data
//#endif
...
void render_reset(void)
{
    ...
    core_memset(bitmap.data, 0, bitmap.pitch * bitmap.height);
    ...
    core_memset(cache, 0, sizeof(cache));
...
void render_line(int line)
{
    ...
    if( (!(vdp.reg[1] & 0x40)) || (((vdp.reg[2] & 1) == 0) && (IS_SMS))) {
        core_memset(linebuf + (vp_hstart << 3), BACKDROP_COLOR, BMP_WIDTH);
    }
    else{
        /* Draw background */
        render_bg(line);
        ...
        if(vdp.reg[0] & 0x20){
            core_memset(linebuf, BACKDROP_COLOR, 8);
        }
    }
...
void render_bg_sms(int line)
{
    ...
    u16 attr;
    u16 *nt = (u16 *)&vdp.vram[vdp.ntab + ((v_line >> 3) << 6)];
    int nt_scroll = (hscroll >> 3);
    int shift = (hscroll & 7);
    u32 atex_mask;
    u32 *cache_ptr;
#if 1
    u8 *linebuf_ptr = (u8 *)&linebuf[0 - shift];
#else
    u32 *linebuf_ptr = (u32 *)&linebuf[0 - shift];
#endif
...
#if 1
            linebuf[(0 - shift) + (x*4)  ] = ((c) | (a));
            linebuf[(0 - shift) + (x*4)+1] = ((c) | (a))>>8;
            linebuf[(0 - shift) + (x*4)+2] = ((c) | (a))>>16;
            linebuf[(0 - shift) + (x*4)+3] = ((c) | (a))>>24;
#else
    ...
    for(; column < vp_hend; column += 1){
        ...
        if((vdp.reg[0] & 0x80) && (!locked) && (column >= 24)){
            locked = 1;
            v_row = (line & 7) << 3;
            nt = (u16 *)&vdp.vram[((vdp.reg[2] << 10) & 0x3800) + ((line >> 3) << 6)];
        }
        ...
        cache_ptr = (u32 *)&cache[((attr & 0x7FF) << 6) | (v_row)];
        
#if 1
        linebuf_ptr[(column*8)+0] = (cache_ptr[0] | (atex_mask));
        linebuf_ptr[(column*8)+1] = (cache_ptr[0] | (atex_mask))>>8;
        linebuf_ptr[(column*8)+2] = (cache_ptr[0] | (atex_mask))>>16;
        linebuf_ptr[(column*8)+3] = (cache_ptr[0] | (atex_mask))>>24;

        linebuf_ptr[(column*8)+4] = (cache_ptr[1] | (atex_mask));
        linebuf_ptr[(column*8)+5] = (cache_ptr[1] | (atex_mask))>>8;
        linebuf_ptr[(column*8)+6] = (cache_ptr[1] | (atex_mask))>>16;
        linebuf_ptr[(column*8)+7] = (cache_ptr[1] | (atex_mask))>>24;
#else
...
void render_bg_gg(int line)
{
    ...
    u32 *cache_ptr;
#if 1
	u8 *linebuf_ptr = (u8*)&linebuf[0 - (hscroll & 7)];
#else
    u32 *linebuf_ptr = (u32*)&linebuf[0 - (hscroll & 7)];
#endif
    /* Draw a line of the background */
...
void render_obj(int line)
{
    ...
    for(i = 0; i < 64; i += 1){
        ...
        if((line >= yp) && (line < (yp + height))){
            ...
            int start = 0;
            int end = width;
            int xp = st[0x80 + (i << 1)]; /* Sprite X position */
            int n = st[0x81 + (i << 1)];  /* Pattern name */

            
            count += 1;                   /* Bump sprite count */
            ...
            if(vdp.reg[6] & 0x04) n |= 0x0100;    /* Add MSB of pattern name */
            if(vdp.reg[1] & 0x02) n &= 0x01FE;    /* Mask LSB for 8x16 sprites */

            /* Point to offset in line buffer */
            linebuf_ptr = (u8 *)&linebuf[xp];
            ...
            if(vdp.reg[1] & 0x01){
                int x;
                u8 *cache_ptr = (u8 *)&cache[(n << 6) | (((line - yp) >> 1) << 3)];
            ...
            {
                int x;
                u8 *cache_ptr = (u8 *)&cache[(n << 6) | ((line - yp) << 3)];
...
{
    int i,length = BMP_WIDTH;
    int ofs = BMP_X_OFFSET;
    u16 *p = (u16 *)&bitmap.data[(line * bitmap.pitch)*2+(ofs<<1)];
