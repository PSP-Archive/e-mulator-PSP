...
byte*      cheat_map   = 0; //[0x10000];
int*       cheat_map_n = 0; //[0x10000];
cheat_dat* cheats      = 0; //[MAX_CHEATS];

int nCheats;
int cheat_enable = 0;

void malloc_cheatmap(void)
{
	if(!cheat_map)   cheat_map   = (byte*)     HAL_mem_malloc(0x10000 * sizeof(byte));
    if(!cheat_map_n) cheat_map_n = (int*)      HAL_mem_malloc(0x10000 * sizeof(int));
    if(!cheats)      cheats      = (cheat_dat*)HAL_mem_malloc(MAX_CHEATS * sizeof(cheat_dat));
}

void free_cheatmap(void)
{
    HAL_mem_free(cheat_map);   cheat_map=0;
    HAL_mem_free(cheat_map_n); cheat_map_n=0;
    HAL_mem_free(cheats);      cheats=0;
}
