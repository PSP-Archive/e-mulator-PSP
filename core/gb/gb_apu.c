...
void snd_reset()
{
	...
	core_memcpy(&snd_stat_cpy,&snd_stat,sizeof(snd_stat));
{
	...
	if (rom_get_info()->gb_type<=2) // 初期型GB & SGB
		core_memcpy(snd_mem+20,gb_init_wav,16);
	else if (rom_get_info()->gb_type>=3) // GBC
		core_memcpy(snd_mem+20,gbc_init_wav,16);
}
}
...
{
#if ! 1
//	core_memcpy(&snd_stat_tmp,&snd_stat,sizeof(snd_stat));
//	core_memcpy(&snd_stat,&snd_stat_cpy,sizeof(snd_stat_cpy));
#endif
...
#if 1       //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			*bufL++ = tmp_l * snd_stat.master_vol[0] >> 8;
			*bufR++ = tmp_r * snd_stat.master_vol[1] >> 8;
#else       //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	core_memcpy(&snd_stat_cpy,&snd_stat,sizeof(snd_stat));
	core_memcpy(&snd_stat,&snd_stat_tmp,sizeof(snd_stat));
}
