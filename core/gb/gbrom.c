...
//	}
	...
	core_memcpy(info.cart_name,buf+0x134,16);
	info.cart_name[16]='\0';
	info.cart_name[17]='\0';
	...
	if (core_memcmp(info.cart_name,momocol_title,16)==0){
	...
	if(buf[0x143]&0x80)       org_gbtype = 3;
	else if(buf[0x146]==0x03) org_gbtype = 2;
	else                      org_gbtype = 1;
...
//	core_memcpy(dat,buf,size);
	dat = buf;
	first_page = dat;

	sram=(byte*)HAL_mem_malloc(rom_get_sram_size());
	if (ram_size) core_memcpy(sram,ram,ram_size&0xffffff00);
	else          core_memset(sram,0x00,rom_get_sram_size());
//	sram = ram;
