...
//static u32 cpu_type;
static u16 prefix_base;	/* base address of the latest prefix segment */
char seg_prefix;		/* prefix segment indicator */

static int clk_Stall = 8;
...
{                   /* eight general registers */
    u16 w[8];    /* viewed as 16 bits registers */
    u8  b[16];   /* or as 8 bit registers */
} necbasicregs;

#define I_SREG4(idx)  (I.sregs[(idx)])

typedef struct
{
    u8 tag_cpu[4]; // for StateTag
    
	necbasicregs regs;
 	u16	sregs[4];
	u16	ip;
	s32	SignVal;
    u32  AuxVal, OverVal, ZeroVal,  ParityVal; /* 0 or non-0 valued flags */
    u32  CarryVal; 
    
	u8	TF, IF, DF, MF; 	/* 0 or 1 valued flags */	/* OB[19.07.99] added Mode Flag V30 */
	u32	int_vector;
	u32	pending_irq;
	u32	nmi_state;
	u32	irq_state;
...
static short nec_ICount; // short:2081, int:2112
...
int nec_getRegSize(void)
{
    return sizeof(I);
}

// パイプライン ストール時のペナルティサイクル設定関数
...
void nec_reset(void *param)
{
    ...
    core_memset( &I, 0, sizeof(I) );
    core_memcpy( &I.tag_cpu,"CPU\0",4);
...
void nec_interrupt(unsigned int_num,u8 md_flag)
{
...
OP( 0x06, i_push_es  ) { PUSH(I.sregs[ES]);	CLK(2); PSTALL(8); }
OP( 0x07, i_pop_es   ) { POP(I.sregs[ES]);	CLK(3);	PSTALL(8); }
...
OP( 0x0f, i_pre_nec  ) { u32 ModRM, tmp, tmp2; /* pop cs at V30MZ? */
	switch (FETCH) {
		case 0x10 : BITOP_BYTE;	CLKS(3); tmp2 = I.regs.b[CL] & 0x7;	I.ZeroVal = (tmp & (1<<tmp2)) ? 1 : 0;	I.CarryVal=I.OverVal=0; break; /* Test */
		case 0x11 : BITOP_WORD;	CLKS(3); tmp2 = I.regs.b[CL] & 0xf;	I.ZeroVal = (tmp & (1<<tmp2)) ? 1 : 0;	I.CarryVal=I.OverVal=0; break; /* Test */
		case 0x12 : BITOP_BYTE;	CLKS(5); tmp2 = I.regs.b[CL] & 0x7;	tmp &= ~(1<<tmp2);	PutbackRMByte(ModRM,tmp);	break; /* Clr */
		case 0x13 : BITOP_WORD;	CLKS(5); tmp2 = I.regs.b[CL] & 0xf;	tmp &= ~(1<<tmp2);	PutbackRMWord(ModRM,tmp);	break; /* Clr */
		case 0x14 : BITOP_BYTE;	CLKS(4); tmp2 = I.regs.b[CL] & 0x7;	tmp |= (1<<tmp2);	PutbackRMByte(ModRM,tmp);	break; /* Set */
		case 0x15 : BITOP_WORD;	CLKS(4); tmp2 = I.regs.b[CL] & 0xf;	tmp |= (1<<tmp2);	PutbackRMWord(ModRM,tmp);	break; /* Set */
		case 0x16 : BITOP_BYTE;	CLKS(4); tmp2 = I.regs.b[CL] & 0x7;	BIT_NOT;			PutbackRMByte(ModRM,tmp);	break; /* Not */
		case 0x17 : BITOP_WORD;	CLKS(4); tmp2 = I.regs.b[CL] & 0xf;	BIT_NOT;			PutbackRMWord(ModRM,tmp);	break; /* Not */

		case 0x18 : BITOP_BYTE;	CLKS(4); tmp2 = (FETCH) & 0x7;	I.ZeroVal = (tmp & (1<<tmp2)) ? 1 : 0;	I.CarryVal=I.OverVal=0; break; /* Test */
		case 0x19 : BITOP_WORD;	CLKS(4); tmp2 = (FETCH) & 0xf;	I.ZeroVal = (tmp & (1<<tmp2)) ? 1 : 0;	I.CarryVal=I.OverVal=0; break; /* Test */
		case 0x1a : BITOP_BYTE;	CLKS(6); tmp2 = (FETCH) & 0x7;	tmp &= ~(1<<tmp2);		PutbackRMByte(ModRM,tmp);	break; /* Clr */
		case 0x1b : BITOP_WORD;	CLKS(6); tmp2 = (FETCH) & 0xf;	tmp &= ~(1<<tmp2);		PutbackRMWord(ModRM,tmp);	break; /* Clr */
		case 0x1c : BITOP_BYTE;	CLKS(5); tmp2 = (FETCH) & 0x7;	tmp |= (1<<tmp2);		PutbackRMByte(ModRM,tmp);	break; /* Set */
		case 0x1d : BITOP_WORD;	CLKS(5); tmp2 = (FETCH) & 0xf;	tmp |= (1<<tmp2);		PutbackRMWord(ModRM,tmp);	break; /* Set */
		case 0x1e : BITOP_BYTE;	CLKS(5); tmp2 = (FETCH) & 0x7;	BIT_NOT;				PutbackRMByte(ModRM,tmp);	break; /* Not */
		case 0x1f : BITOP_WORD;	CLKS(5); tmp2 = (FETCH) & 0xf;	BIT_NOT;				PutbackRMWord(ModRM,tmp);	break; /* Not */

		case 0x20 :	ADD4S; CLKS(7); break;
		case 0x22 :	SUB4S; CLKS(7); break;
		case 0x26 :	CMP4S; CLKS(7); break;
...
OP( 0x17, i_pop_ss   ) { POP(I.sregs[SS]);		CLK(3);	}
...
OP( 0x26, i_es       ) { seg_prefix=TRUE; prefix_base=I_SREG4(ES); CLK(1); nec_instruction[FETCHOP](); seg_prefix=FALSE; }
...
OP( 0x2e, i_cs       ) { seg_prefix=TRUE;	prefix_base=I_SREG4(CS);	CLK(1);		nec_instruction[FETCHOP]();	seg_prefix=FALSE; }
...
OP( 0x32, i_xor_r8b  ) { DEF_r8b;	XORB;	RegByte(ModRM)=dst;			CLKM(2,1); 		} // ok
...
OP( 0x36, i_ss       ) { seg_prefix=TRUE;	prefix_base=I_SREG4(SS);	CLK(1);		nec_instruction[FETCHOP]();	seg_prefix=FALSE; }
...
OP( 0x3e, i_ds       ) { seg_prefix=TRUE;	prefix_base=I_SREG4(DS);	CLK(1);		nec_instruction[FETCHOP]();	seg_prefix=FALSE; }
...
OP( 0x61, i_popa  ) {
	...
	POP(I.regs.w[IX]);
	POP(I.regs.w[BP]);
    I.regs.w[SP]+=2; // { unsigned tmp; POP(tmp); }
	POP(I.regs.w[BW]);
	POP(I.regs.w[DW]);
	POP(I.regs.w[CW]);
	POP(I.regs.w[AW]);
...
OP( 0x64, i_repnc  ) {
    u32 next = FETCHOP;	u16 c = I.regs.w[CW];
    switch(next) { /* Segments */
      case 0x26:	seg_prefix=TRUE;	prefix_base=I_SREG4(ES);	next = FETCHOP;	CLK(2); break;
      case 0x2e:	seg_prefix=TRUE;	prefix_base=I_SREG4(CS);	next = FETCHOP;	CLK(2); break;
      case 0x36:	seg_prefix=TRUE;	prefix_base=I_SREG4(SS);	next = FETCHOP;	CLK(2); break;
      case 0x3e:	seg_prefix=TRUE;	prefix_base=I_SREG4(DS);	next = FETCHOP;	CLK(2); break;
    }
...
OP( 0x68, i_push_d16 ) { u32 tmp;	FETCHWORD(tmp); PUSH(tmp);	CLK(1);	}
OP( 0x69, i_imul_d16 ) { u32 tmp;	DEF_r16w; 	FETCHWORD(tmp); dst = (s32)((s16)src)*(s32)((s16)tmp); I.CarryVal = I.OverVal = (((s32)dst) >> 15 != 0) && (((s32)dst) >> 15 != -1);     RegWord(ModRM)=(WORD)dst;     CLKM(4,3);}
OP( 0x6a, i_push_d8  ) { u32 tmp = (WORD)((s16)((s8)FETCH)); 	PUSH(tmp);	CLK(1);	}
OP( 0x6b, i_imul_d8  ) { u32 src2; DEF_r16w; src2= (WORD)((s16)((s8)FETCH)); dst = (s32)((s16)src)*(s32)((s16)src2); I.CarryVal = I.OverVal = (((s32)dst) >> 15 != 0) && (((s32)dst) >> 15 != -1); RegWord(ModRM)=(WORD)dst; CLKM(4,3); }
...
OP( 0x6e, i_outsb    ) { write_port(I.regs.w[DW],GetMemB(DS,I.regs.w[IX])); I.regs.w[IX]+= -2 * I.DF + 1; CLK(7); } // ok
...
OP( 0x70, i_jo      ) { JMPCLK( OF,1);				}
OP( 0x71, i_jno     ) { JMPCLK(!OF,1);				}
OP( 0x72, i_jc      ) { JMPCLK( CF,1);				}
OP( 0x73, i_jnc     ) { JMPCLK(!CF,1);				}
OP( 0x74, i_jz      ) { JMPCLK( ZF,1);				}
OP( 0x75, i_jnz     ) { JMPCLK(!ZF,1);				}
OP( 0x76, i_jce     ) { JMPCLK(CF || ZF,1);			}
OP( 0x77, i_jnce    ) { JMPCLK(!(CF || ZF),1);		}
OP( 0x78, i_js      ) { JMPCLK( SF,1);				}
OP( 0x79, i_jns     ) { JMPCLK(!SF,1);				}
OP( 0x7a, i_jp      ) { JMPCLK( PF,1);				}
OP( 0x7b, i_jnp     ) { JMPCLK(!PF,1);				}
OP( 0x7c, i_jl      ) { JMPCLK((SF!=OF)&&(!ZF),1);	}
OP( 0x7d, i_jnl     ) { JMPCLK((ZF)||(SF==OF),1);	}
OP( 0x7e, i_jle     ) { JMPCLK((ZF)||(SF!=OF),1); 	}
OP( 0x7f, i_jnle    ) { JMPCLK((SF==OF)&&(!ZF),1);	}
...
OP( 0x80, i_80pre ) { u32 dst, src; GetModRM; dst = GetRMByte(ModRM); src = FETCH;
	CLKM(3,1)
	switch (ModRM & 0x38) {
...
OP( 0x81, i_81pre   ) { u32 dst, src; GetModRM; dst = GetRMWord(ModRM); src = FETCH; src+= (FETCH << 8);
	CLKM(3,1)
    switch (ModRM & 0x38) {
...
OP( 0x82, i_82pre   ) { u32 dst, src; GetModRM; dst = GetRMByte(ModRM); src = (BYTE)((s8)FETCH);
	CLKM(3,1)
	switch (ModRM & 0x38) {
	    case 0x00: ADDB;			PutbackRMByte(ModRM,dst);	break;
	    case 0x08: ORB;				PutbackRMByte(ModRM,dst);	break;
...
OP( 0x83, i_83pre   ) { u32 dst, src; GetModRM; dst = GetRMWord(ModRM); src = (WORD)((s16)((s8)FETCH));
	CLKM(3,1)
    switch (ModRM & 0x38) {
	    case 0x00: ADDW;			PutbackRMWord(ModRM,dst);	break;
	    case 0x08: ORW;				PutbackRMWord(ModRM,dst);	break;
...
OP( 0x88, i_mov_br8   ) { u8  src; GetModRM; src = RegByte(ModRM); 	PutRMByte(ModRM,src); CLKM(1,1); }
OP( 0x89, i_mov_wr16  ) { u16 src; GetModRM; src = RegWord(ModRM); 	PutRMWord(ModRM,src); CLKM(1,1); }
OP( 0x8a, i_mov_r8b   ) { u8  src; GetModRM; src = GetRMByte(ModRM);	RegByte(ModRM)=src; CLKM(1,1); }
OP( 0x8b, i_mov_r16w  ) { u16 src; GetModRM; src = GetRMWord(ModRM);	RegWord(ModRM)=src; CLKM(1,1); }
OP( 0x8c, i_mov_wsreg ) { GetModRM; PutRMWord(ModRM,I.sregs[(ModRM & 0x38) >> 3]); CLKM(1,1); }

OP( 0x8d, i_lea       ) { u16 ModRM = FETCH; (void)(*GetEA[ModRM])(); RegWord(ModRM)=EO; 	CLK(1); }
OP( 0x8e, i_mov_sregw ) { u16 src; GetModRM; src = GetRMWord(ModRM); CLKM(3,2); // ok
    switch (ModRM & 0x38) {
...
OP( 0x8f, i_popw ) { u16 tmp; GetModRM; POP(tmp); PutRMWord(ModRM,tmp); CLKM(3,1); }
OP( 0x90, i_nop  ) { CLK(1); }
...
OP( 0x9a, i_call_far  ) { u32 tmp, tmp2;	FETCHWORD(tmp); FETCHWORD(tmp2); PUSH(I.sregs[CS]); PUSH(I.ip); I.ip = (WORD)tmp; I.sregs[CS] = (WORD)tmp2; CLK(10); PSTALL(8); }

OP( 0x9b, i_wait      ) { ; }
OP( 0x9c, i_pushf     ) { PUSH( CompressFlags() ); CLK(2); }
OP( 0x9d, i_popf      ) { u32 tmp; POP(tmp); ExpandFlags(tmp); CLK(3); }
OP( 0x9e, i_sahf      ) { u32 tmp = (CompressFlags() & 0xff00) | (I.regs.b[AH] & 0xd5); ExpandFlags(tmp); CLK(4); }
OP( 0x9f, i_lahf      ) { I.regs.b[AH] = CompressFlags() & 0xff; CLK(2); }

OP( 0xa0, i_mov_aldisp ) { u32 addr; FETCHWORD(addr); I.regs.b[AL] = GetMemB(DS, addr); CLK(1); }
OP( 0xa1, i_mov_axdisp ) { u32 addr; FETCHWORD(addr); I.regs.b[AL] = GetMemB(DS, addr); I.regs.b[AH] = GetMemB(DS, (addr+1)&0xffff); CLK(1); }
OP( 0xa2, i_mov_dispal ) { u32 addr; FETCHWORD(addr); PutMemB(DS, addr, I.regs.b[AL]);  CLK(1); }
OP( 0xa3, i_mov_dispax ) { u32 addr; FETCHWORD(addr); PutMemB(DS, addr, I.regs.b[AL]);  PutMemB(DS, (addr+1)&0xffff, I.regs.b[AH]); CLK(1); }

OP( 0xa4, i_movsb      ) { 
	u32 tmp = GetMemB(DS,I.regs.w[IX]); 
...
OP( 0xa5, i_movsw      ) { 
	u32 tmp = GetMemW(DS,I.regs.w[IX]); 
...
OP( 0xa6, i_cmpsb      ) { u32 src = GetMemB(ES, I.regs.w[IY]); u32 dst = GetMemB(DS, I.regs.w[IX]); SUBB; I.regs.w[IY] += -2 * I.DF + 1; I.regs.w[IX] += -2 * I.DF + 1; CLK(6); }
OP( 0xa7, i_cmpsw      ) { u32 src = GetMemW(ES, I.regs.w[IY]); u32 dst = GetMemW(DS, I.regs.w[IX]); SUBW; I.regs.w[IY] += -4 * I.DF + 2; I.regs.w[IX] += -4 * I.DF + 2; CLK(6); }
...
OP( 0xae, i_scasb      ) { u32 src = GetMemB(ES, I.regs.w[IY]); 	u32 dst = I.regs.b[AL]; SUBB; I.regs.w[IY] += -2 * I.DF + 1; CLK(4);  }
OP( 0xaf, i_scasw      ) { u32 src = GetMemW(ES, I.regs.w[IY]); 	u32 dst = I.regs.w[AW]; SUBW; I.regs.w[IY] += -4 * I.DF + 2; CLK(4); }

OP( 0xb0, i_mov_ald8  ) { I.regs.b[AL] = FETCH;	CLK(1); } // ok
OP( 0xb1, i_mov_cld8  ) { I.regs.b[CL] = FETCH; CLK(1); } // ok
OP( 0xb2, i_mov_dld8  ) { I.regs.b[DL] = FETCH; CLK(1); } // ok
OP( 0xb3, i_mov_bld8  ) { I.regs.b[BL] = FETCH; CLK(1); } // ok
OP( 0xb4, i_mov_ahd8  ) { I.regs.b[AH] = FETCH; CLK(1); } // ok
OP( 0xb5, i_mov_chd8  ) { I.regs.b[CH] = FETCH; CLK(1); } // ok
OP( 0xb6, i_mov_dhd8  ) { I.regs.b[DH] = FETCH; CLK(1); } // ok
OP( 0xb7, i_mov_bhd8  ) { I.regs.b[BH] = FETCH;	CLK(1); } // ok

OP( 0xb8, i_mov_axd16 ) { I.regs.b[AL] = FETCH;	 I.regs.b[AH] = FETCH;	CLK(1); } // ok
OP( 0xb9, i_mov_cxd16 ) { I.regs.b[CL] = FETCH;	 I.regs.b[CH] = FETCH;	CLK(1); } // ok
OP( 0xba, i_mov_dxd16 ) { I.regs.b[DL] = FETCH;	 I.regs.b[DH] = FETCH;	CLK(1); } // ok
OP( 0xbb, i_mov_bxd16 ) { I.regs.b[BL] = FETCH;	 I.regs.b[BH] = FETCH;	CLK(1); } // ok
OP( 0xbc, i_mov_spd16 ) { I.regs.b[SPL] = FETCH; I.regs.b[SPH] = FETCH;	CLK(1); } // ok
OP( 0xbd, i_mov_bpd16 ) { I.regs.b[BPL] = FETCH; I.regs.b[BPH] = FETCH; CLK(1); } // ok
OP( 0xbe, i_mov_sid16 ) { I.regs.b[IXL] = FETCH; I.regs.b[IXH] = FETCH;	CLK(1); } // ok
OP( 0xbf, i_mov_did16 ) { I.regs.b[IYL] = FETCH; I.regs.b[IYH] = FETCH;	CLK(1); } // ok
...
OP( 0xc2, i_ret_d16  ) { u32 count = FETCH; count += FETCH << 8; POP(I.ip); I.regs.w[SP]+=count; CLK(6); PSTALL(8); }
OP( 0xc3, i_ret      ) { POP(I.ip); CLK(6); PSTALL(8); }
...
OP( 0xca, i_retf_d16  ) { u32 count = FETCH; count += FETCH << 8; POP(I.ip); POP(I.sregs[CS]); I.regs.w[SP]+=count; CLK(9); PSTALL(8); }
OP( 0xcb, i_retf      ) { POP(I.ip); POP(I.sregs[CS]); CLK(8); PSTALL(8); }
...
OP( 0xcf, i_iret      ) { POP(I.ip); POP(I.sregs[CS]); i_popf(); CLK(10); PSTALL(8); }

OP( 0xd0, i_rotshft_b ) {
	u32 src, dst; GetModRM; src = (u32)GetRMByte(ModRM); dst=src;
	CLKM(3,1);
    switch (ModRM & 0x38) {
...
OP( 0xd1, i_rotshft_w ) {
	u32 src, dst; GetModRM; src = (u32)GetRMWord(ModRM); dst=src;
	CLKM(3,1);
	switch (ModRM & 0x38) {
...
OP( 0xd2, i_rotshft_bcl ) {
	u32 src, dst; u8 c; GetModRM; src = (u32)GetRMByte(ModRM); dst=src;
	c=I.regs.b[CL];
	CLKM(5,3);
	c&=0x1f;
	if (c) {
...
OP( 0xd3, i_rotshft_wcl ) {
	u32 src, dst; u8 c; GetModRM; src = (u32)GetRMWord(ModRM); dst=src;
	c=I.regs.b[CL];
	c&=0x1f;
	CLKM(5,3);
    if (c) {
...
OP( 0xd4, i_aam    ) { u32 mult=FETCH; mult=0; I.regs.b[AH] = I.regs.b[AL] / 10; I.regs.b[AL] %= 10; SetSZPF_Word(I.regs.w[AW]); CLK(17); }
OP( 0xd5, i_aad    ) { u32 mult=FETCH; mult=0; I.regs.b[AL] = I.regs.b[AH] * 10 + I.regs.b[AL]; I.regs.b[AH] = 0; SetSZPF_Byte(I.regs.b[AL]); CLK(6); }
OP( 0xd6, i_setalc ) { I.regs.b[AL] = (CF)?0xff:0x00; CLK(3);  } /* nop at V30MZ? */
OP( 0xd7, i_trans  ) { u32 dest = (I.regs.w[BW]+I.regs.b[AL])&0xffff; I.regs.b[AL] = GetMemB(DS, dest); CLK(5); }
OP( 0xd8, i_fpo    ) { GetModRM; CLK(3);	 } /* nop at V30MZ? */

OP( 0xe0, i_loopne ) { s8 disp = (s8)FETCH; I.regs.w[CW]--; if (!ZF && I.regs.w[CW]) { I.ip = (WORD)(I.ip+disp);  CLK(6); PSTALL(8); } else CLK(3); }
OP( 0xe1, i_loope  ) { s8 disp = (s8)FETCH; I.regs.w[CW]--; if ( ZF && I.regs.w[CW]) { I.ip = (WORD)(I.ip+disp);  CLK(6); PSTALL(8); } else CLK(3); }
OP( 0xe2, i_loop   ) { s8 disp = (s8)FETCH; I.regs.w[CW]--; if (I.regs.w[CW]) { I.ip = (WORD)(I.ip+disp);  CLK(5); } else CLK(2); }
OP( 0xe3, i_jcxz   ) { s8 disp = (s8)FETCH; if (I.regs.w[CW] == 0) { I.ip = (WORD)(I.ip+disp);  CLK(4); } else CLK(1); }
OP( 0xe4, i_inal   ) { u8 port = FETCH; I.regs.b[AL] = read_port(port); CLK(6);	}
OP( 0xe5, i_inax   ) { u8 port = FETCH; I.regs.b[AL] = read_port(port); I.regs.b[AH] = read_port(port+1); CLK(6); }
OP( 0xe6, i_outal  ) { u8 port = FETCH; write_port (port, I.regs.b[AL]); CLK(6); }
OP( 0xe7, i_outax  ) { u8 port = FETCH; write_portw(port, I.regs.w[AW]); CLK(6); }
//OP( 0xe6, i_outal  ) { u8 port = FETCH; write_port(port, I.regs.b[AL]); CLK(6); }
//OP( 0xe7, i_outax  ) { u8 port = FETCH; write_port(port, I.regs.b[AL]); write_port(port+1, I.regs.b[AH]); CLK(6);	}

OP( 0xe8, i_call_d16 ) { 
	u32 tmp;
	FETCHWORD(tmp); 
	PUSH(I.ip);
	I.ip = (WORD)(I.ip+(s16)tmp); 
	CLK(5); 
}
OP( 0xe9, i_jmp_d16  ) { u32 tmp; FETCHWORD(tmp); I.ip = (WORD)(I.ip+(s16)tmp); CLK(4); }
OP( 0xea, i_jmp_far  ) { u32 tmp,tmp1; FETCHWORD(tmp); FETCHWORD(tmp1); I.sregs[CS] = (WORD)tmp1; I.ip = (WORD)tmp; CLK(7);  } // ok
OP( 0xeb, i_jmp_d8   ) { int tmp = (int)((s8)FETCH); CLK(4);
	if (tmp==-2 && nec_ICount>0) nec_ICount%=12; /* cycle skip */
	I.ip = (WORD)(I.ip+tmp);
}
OP( 0xec, i_inaldx   ) { I.regs.b[AL] = read_port(I.regs.w[DW]); CLK(6);}
OP( 0xed, i_inaxdx   ) { u32 port = I.regs.w[DW];	I.regs.b[AL] = read_port(port);	I.regs.b[AH] = read_port(port+1); CLK(6); }
OP( 0xee, i_outdxal  ) { write_port(I.regs.w[DW], I.regs.b[AL]); CLK(6);	}
OP( 0xef, i_outdxax  ) { u32 port = I.regs.w[DW];	write_port(port, I.regs.b[AL]);	write_port(port+1, I.regs.b[AH]); CLK(6); }

OP( 0xf0, i_lock     ) { CLK(1); }
...
OP( 0xf2, i_repne    ) { u32 next = FETCHOP; u16 c = I.regs.w[CW];
    switch(next) { /* Segments */
      case 0x26:	seg_prefix=TRUE;	prefix_base=I_SREG4(ES);	next = FETCHOP;	CLK(2); break;
      case 0x2e:	seg_prefix=TRUE;	prefix_base=I_SREG4(CS);	next = FETCHOP;	CLK(2); break;
      case 0x36:	seg_prefix=TRUE;	prefix_base=I_SREG4(SS);	next = FETCHOP;	CLK(2); break;
...
OP( 0xf3, i_repe     ) { 
    ...
    switch(next) {
		case 0x6c:	CLK(5+( 6*c)); if (c) do { THROUGH; i_insb();  c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
		case 0x6d:  CLK(5+( 6*c)); if (c) do { THROUGH; i_insw();  c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0x6e:	CLK(5+( 6*c)); if (c) do { THROUGH; i_outsb(); c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0x6f:  CLK(5+( 6*c)); if (c) do { THROUGH; i_outsw(); c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0xa4:	CLK(5+( 7*c)); if (c) do { THROUGH; i_movsb(); c--; /*CLK( 7);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0xa5:  CLK(5+( 7*c)); if (c) do { THROUGH; i_movsw(); c--; /*CLK( 7);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0xa6:	CLK(5+(10*c)); if (c) do { THROUGH; i_cmpsb(); c--; /*CLK(10);*/ } while (c>0 && ZF==1);	I.regs.w[CW]=c; break;
	    case 0xa7:	CLK(5+(10*c)); if (c) do { THROUGH; i_cmpsw(); c--; /*CLK(10);*/ } while (c>0 && ZF==1);	I.regs.w[CW]=c; break;
	    case 0xaa:	CLK(5+( 6*c)); if (c) do { THROUGH; i_stosb(); c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0xab:  CLK(5+( 6*c)); if (c) do { THROUGH; i_stosw(); c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0xac:	CLK(5+( 6*c)); if (c) do { THROUGH; i_lodsb(); c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0xad:  CLK(5+( 6*c)); if (c) do { THROUGH; i_lodsw(); c--; /*CLK( 6);*/ } while (c>0);	I.regs.w[CW]=c;	break;
	    case 0xae:	CLK(5+( 9*c)); if (c) do { THROUGH; i_scasb(); c--; /*CLK( 9);*/ } while (c>0 && ZF==1);	I.regs.w[CW]=c; break;
	    case 0xaf:	CLK(5+( 9*c)); if (c) do { THROUGH; i_scasw(); c--; /*CLK( 9);*/ } while (c>0 && ZF==1);	I.regs.w[CW]=c; break;
		default:	 nec_instruction[next]();
    }
	seg_prefix=FALSE;
}
...
OP( 0xf6, i_f6pre ) { u32 tmp; u32 uresult,uresult2; s32 result,result2;
    ...
    switch (ModRM & 0x38) {
		...
		case 0x28: result = (s16)((s8)I.regs.b[AL])*(s16)((s8)tmp); I.regs.w[AW]=(WORD)result; I.CarryVal=I.OverVal=(I.regs.b[AH]!=0); CLKM(4,3); break; /* MUL */
...
OP( 0xf7, i_f7pre   ) { u32 tmp,tmp2; u32 uresult,uresult2; s32 result,result2;
    ...
    switch (ModRM & 0x38) {
		...
		case 0x20: uresult = I.regs.w[AW]*tmp; I.regs.w[AW]=uresult&0xffff; I.regs.w[DW]=((u32)uresult)>>16; I.CarryVal=I.OverVal=(I.regs.w[DW]!=0); CLKM(4,3); break; /* MULU */
		case 0x28: result = (s32)((s16)I.regs.w[AW])*(s32)((s16)tmp); I.regs.w[AW]=result&0xffff; I.regs.w[DW]=result>>16; I.CarryVal=I.OverVal=(I.regs.w[DW]!=0); CLKM(4,3); break; /* MUL */
...
OP( 0xfb, i_ei    ) { SetIF(1);	ws_int_check(); CLK(4);	}

OP( 0xfc, i_cld   ) { SetDF(0);			CLK(4);	}
OP( 0xfd, i_std   ) { SetDF(1);			CLK(4);	}
OP( 0xfe, i_fepre ) { u32 tmp, tmp1; GetModRM; tmp=GetRMByte(ModRM);
    switch(ModRM & 0x38) {
...
OP( 0xff, i_ffpre ) { u32 tmp, tmp1; GetModRM; tmp=GetRMWord(ModRM);
    switch(ModRM & 0x38) {
...
void nec_execute(int cycles)
{
    ...
    while(nec_ICount>0) {
		fetch_op = FETCHOP;
        nec_instruction[fetch_op]();
    }
//	return cycles - nec_ICount;
}

void nec_use_count(int clk)
{	
	nec_ICount -= clk;
}
