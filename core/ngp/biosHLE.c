...
{
#ifdef PC32ADR
	u32 nbios = (u32)pc32_adr - (u32)bios;

	nbios = 0x00ff0000 | (nbios-1);

	pc32_adr--;
#else
	//Only works within the bios
	if ((pc32 & 0xFF0000) != 0xFF0000)
		return;
	pc32 --;	//Compensate for processing this instruction.
#endif
...
//              system_message(system_get_string(IDS_POWER));
...
#endif
		{
			...
			for (i = 0; i < 0x800; i++)
			{
				...
				for (j = 0; j < 8; j++, c<<=1)
				{
					...
					data16 = le16toh(*(u16*)(ram + dst));
					data16 <<= 2;
					*(u16*)(ram + dst) = htole16(data16);
	...
	case 0xFF2C86:
		...
		//Restore $pc32 after BIOS-HLE instruction
#ifdef PC32ADR
		pc32_adr = pop32();
#else
		pc32 = pop32();
