...
// Helper Macros
//
//#define SET_Z(m)				{ mZ=(m)?false:1/*true*/; }
//#define SET_N(m)				{ mN=(m&0x80)?1/*true*/:false; }
...
//#define	xADC()\
//{\
//	u8	value=CPU_PEEK(mOperand);\
//	u8	oldA=mA;\
//	if(!mD)\
//	{\
//		SWORD sum=(SWORD)((s8)mA)+(SWORD)((s8)value)+(mC?1:0);\
//		mV=((sum > 127) || (sum < -128));\
...
//{\
//	u8 value=CPU_PEEK(mOperand);\
//	if(mA+0x100-value>0xff) mC=1/*true*/; else mC=FALSE;\
...
//{\
//	u8 value=CPU_PEEK(mOperand);\
//	if(mX+0x100-value>0xff) mC=1/*true*/; else mC=FALSE;\
...
//{\
//	u8 value=CPU_PEEK(mOperand);\
//	if(mY+0x100-value>0xff) mC=1/*true*/; else mC=FALSE;\
...
//	{\
//		u8 value=~(CPU_PEEK(mOperand));\
//		SWORD difference=(SWORD)((s8)mA)+(SWORD)((s8)value)+(mC?1:0);\
