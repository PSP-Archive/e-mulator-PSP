...
/* Stack push/pull */
#define  PUSH(value)             stack[S--] = (u8) (value)
#define  PULL()                  stack[++S]
...
** The rest of the flags are stored as true u8s.
*/
...
static u8 *ram = NULL, *stack = NULL;
static u8 dead_page[NES6502_BANKSIZE];
...
#define  ZP_WRITEBYTE(addr, value)  ram[(addr)] = (u8) (value)

INLINE u32 zp_readword(register u8 address)
{
#ifdef LSB_FIRST
	u32 ret;
...
INLINE u8 bank_readbyte(register u32 address)
{
   return cpu.mem_page[address >> NES6502_BANKSHIFT][address & NES6502_BANKMASK];
}

INLINE u32 bank_readword(register u32 address)
{
#ifdef LSB_FIRST
   ...
   /* TODO: this fails if host architecture doesn't support byte alignment */
	...
	u8 *ptr = cpu.mem_page[address >> NES6502_BANKSHIFT] + (address & NES6502_BANKMASK);
	ret = *(ptr) + (*(ptr+1) << 8);
   return ret;
#else
#ifdef TARGET_CPU_PPC
...
#else
   u32 x = (u32) *(u16 *)(cpu.mem_page[address >> NES6502_BANKSHIFT] + (address & NES6502_BANKMASK));
   return ((x << 8) | (x >> 8)) & 0xFFFF;
#endif /* TARGET_CPU_PPC */
#endif /* LSB_FIRST */
}

INLINE void bank_writebyte(register u32 address, register u8 value)
{
   cpu.mem_page[address >> NES6502_BANKSHIFT][address & NES6502_BANKMASK] = value;
}

/* read a byte of 6502 memory */
static u8 mem_readbyte(u32 address)
{
...
static void mem_writebyte(u32 address, u8 value)
{
   /* RAM */
   if (address < 0x800)
   {
...
void nes6502_setcontext(nes6502_context *context)
{
   ...
   core_memcpy(&cpu, context, sizeof(nes6502_context));
...
void nes6502_getcontext(nes6502_context *context)
{
   ...
   core_memcpy(context, &cpu, sizeof(nes6502_context));
...
u8 nes6502_getbyte(u32 address)
{
   return bank_readbyte(address);
}

/* get number of elapsed cycles */
u32 nes6502_getcycles(u8 reset_flag)
{
   u32 cycles = cpu.total_cycles;
...
int nes6502_execute(int remaining_cycles)
{
   ...
   u32 temp, addr; /* for macros */
   u8 btemp, baddr; /* for macros */
   u8 data;

   /* flags */
   u8 n_flag, v_flag, b_flag;
   u8 d_flag, i_flag, z_flag, c_flag;
