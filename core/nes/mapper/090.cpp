...
STATIC u8 NES_mapper90_MemoryReadLow(u32 addr)
{
	...
	if(addr == 0x5000)
	{
		return (u8)(g_NESmapper.Mapper90.value1*g_NESmapper.Mapper90.value2 & 0x00FF);
	}
	else
	{
		return (u8)(addr >> 8);
...
STATIC void NES_mapper90_Sync_Mirror()
{
	...
	for(i = 0; i < 4; i++)
	{
		nam_bank[i] = ((u32)g_NESmapper.Mapper90.nam_high_reg[i] << 8) | (u32)g_NESmapper.Mapper90.nam_low_reg[i];
	}
...
STATIC void NES_mapper90_Sync_Chr_Banks()
{
	...
	for(i = 0; i < 8; i++)
	{
		chr_bank[i] = ((u32)g_NESmapper.Mapper90.chr_high_reg[i] << 8) | (u32)g_NESmapper.Mapper90.chr_low_reg[i];
	}
...
STATIC void NES_mapper90_Sync_Prg_Banks()
{
	...
	case 0:
		if(g_NESmapper.Mapper90.mode&4){
			u32 bn = g_NESmapper.Mapper90.prg_reg[3]<<2;
