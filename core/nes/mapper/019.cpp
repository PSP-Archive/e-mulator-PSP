STATIC void NES_mapper19_Init();
STATIC void NES_mapper19_Reset();
STATIC u8 NES_mapper19_MemoryReadLow(u32 addr);
STATIC void NES_mapper19_MemoryWriteLow(u32 addr, u8 data);
STATIC void NES_mapper19_MemoryWrite(u32 addr, u8 data);
STATIC void NES_mapper19_HSync(u32 scanline);
...
STATIC u8 NES_mapper19_MemoryReadLow(u32 addr)
{
	if(addr == 0x4800)
	{
		...
		{
			u8 retval =  NES_ReadSaveRAM(g_NESmapper.Mapper19.regs[2] & 0x7F);
	...
	{
		return (u8)(g_NESmapper.Mapper19.irq_counter & 0x00FF);
	}
	else if ((addr & 0xF800) == 0x5800) // addr $5800-$5FFF
	{
		return (u8)((g_NESmapper.Mapper19.irq_counter & 0x7F00) >> 8);
	}
	else
	{
		return (u8)(addr >> 8);
...
STATIC void NES_mapper19_MemoryWriteLow(u32 addr, u8 data)
{
	...
	case 0x5800: // addr $5000-$57FF
		{
			g_NESmapper.Mapper19.irq_counter = (g_NESmapper.Mapper19.irq_counter & 0x00FF) | ((u32)(data & 0x7F) << 8);
...
STATIC void NES_mapper19_HSync(u32 scanline)
{
	...
	{
		if(g_NESmapper.Mapper19.irq_counter >= (u32)(0x7FFF - g_NESmapper.Mapper19.irq_sn))
		{
