...
static s32 apu_rectangle(rectangle_t *chan)
{
   s32 output;
...
static s32 apu_triangle(triangle_t *chan)
{
#ifdef APU_YANO
   static double val;
...
static s32 apu_noise(noise_t *chan)
{
   s32 outvol;

#ifdef APU_YANO
   static s32 noise_bit;
   double total;
   double sample_weight;
#else /* !APU_YANO */
...
static void apu_regwrite(u32 address, u8 value)
{
   int chan;
   ...
   case APU_WRE2:
      ...
      g_apu_t.apus.dmc.cached_addr = 0xC000 + (u16) (value << 6);
      break;
...
void nes_apu_write(u32 address, u8 value)
{
    apudata_t d;
...
void nes_apu_write_cur(u32 address, u8 value)
{
   /* for sync read $4015 */
   int chan;
   switch (address)
...
void ex_write(u32 address, u8 value)
{
   apudata_t d;
   d.timestamp = NES6502_GetCycles(FALSE);
   d.address = address;
...
void nes_apu_process(s32 *buffer, int num_samples)
{
    apudata_t *d;
    u32 elapsed_cycles;
    static s32 prev_sample = 0;
    s32 next_sample, accum=0;
    int i;

    /* grab it, keep it local for speed */
    elapsed_cycles = (u32) g_apu_t.elapsed_cycles;
...
void nes_apu_reset_apus(APUSOUND *apus)
{
   int i;

   // Reset rectangles
   for ( i=0; i<2; i++) {
      core_memset(&apus->rectangle[i], 0, sizeof(apus->rectangle[i]));
   }
   apus->rectangle[0].sweep_complement = TRUE;
   apus->rectangle[1].sweep_complement = FALSE;
   core_memset(&apus->triangle, 0, sizeof(apus->triangle));
#ifdef APU_YANO
   apus->triangle.ideal_triangle = apus->triangle.ideal_triangle;
#endif
   core_memset(&apus->noise, 0, sizeof(apus->noise));
   core_memset(&apus->dmc, 0, sizeof(apus->dmc));
}
...
void nes_apu_reset(void)
{
   ...
   core_memset(&g_apu_t.queue, 0, APUQUEUE_SIZE * sizeof(apudata_t));
   g_apu_t.q_head = g_apu_t.q_tail = 0;

   core_memset(&g_apu_t.ex_queue, 0, APUQUEUE_SIZE * sizeof(apudata_t));
   g_apu_t.ex_q_head = g_apu_t.ex_q_tail = 0;
   g_apu_t.ex_chip = 0;

   // added by T.Yano
   nes_apu_reset_apus( &g_apu_t.apus);
   ...
   for (address = 0x4000; address <= 0x4013; address++)
   {
      ...
      nes_apu_write_cur(address, 0);
   }

#ifdef NSF_PLAYER
   apu_regwrite(0x400C, 0x10); /* silence noise channel on NSF start */
   apu_regwrite(0x4015, 0x0F);
   nes_apu_write_cur(0x400c, 0x10);
   nes_apu_write_cur(0x4015, 0x0F);
#else /* !NSF_PLAYER */
   apu_regwrite(0x4015, 0x00);
   nes_apu_write_cur(0x4015, 0x00);
#endif /* !NSF_PLAYER */
...
apu_t *apu_create(int sample_rate, int refresh_rate, int frag_size, int sample_bits)
{
   ...
   core_memset(&g_apu_t, 0x00, sizeof(g_apu_t));
   ...
   g_apu_t.process = nes_apu_process;
...
u8 sync_dmc_register(u32 cpu_cycles)
{
   u8 irq_occurred = FALSE;
