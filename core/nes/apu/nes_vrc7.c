...
//static u32 amtbl[1 << AM_BITS];
//static u32 pmtbl[1 << PM_BITS];
//static u32 sintbl[1 << PG_BITS];
//static u32 sintbld[1 << PG_BITS];
// セット済みテーブル
#include "vrc7_tbl.h"
...
__inline static u32 PGCALC(OPLL_OP *pop)
{
	if (pop->pg_vib)
	{
...
__inline static void OPKEY(OPLL_OP *pop, u8 key)
{
	if (key)
	{
		pop->eg_mode = EGM_A;
...
__inline static u32 EGCALC(OPLL_OP *pop)
{
	u32 egout = pop->tl << (LOG_BITS - 3);
...
__inline static void EGSET2KSR(OPLL_OP *pop, u8 freqh)
{
	u8 ksr = (freqh & 0x0F) >> (3 - pop->eg_ks);
	if (pop->eg_ar)
	{
		u8 arr = pop->eg_ar + ksr;
		if (arr > 63) arr = 63;
		pop->eg_arr = (15 - (arr >> 2));
	}
	else
...
__inline static s32 CHCALC(OPLL_CH *pch)
{
	if (pch->update)
	{
		...
		if (pch->update & OPLL_UPDATE_FREQH)
		{
			...
			{
				pch->key ^= 0x10;
				OPKEY(&pch->op[0], (u8)(pch->key & 0x10));
				OPKEY(&pch->op[1], (u8)(pch->key & 0x10));
			}
...
__inline static s32 LFOCALC(OPLL_LFO *pch)
{
	pch->cycles -= pch->cps;
	while (pch->cycles < 0)
	{
...
static s32 OPLLSoundRender(void)
{
	s32 accum = 0;
	u8 ch;
#if OPLL_CH_MAX == 9
	u8 chmax = (opll_sound.rhythmc & 0x20) ? 6 : 9;
#else
	const u8 chmax = 6;	/* VRCVII */
#endif
...
static void OPLLSoundVolume(u32 volume)
{
	opll_sound.mastervolume = (volume << (LOG_BITS - 8)) << 1;
}

static void OPLLSoundWriteAddr(u32 address, u8 value)
{
	opll_sound.adr = value;
}

static void OPLLSoundWriteData(u32 address, u8 value)
{
...
#endif
			{
				if(opll_sound.ch[ch].toneno != (value >> 4))
				{
					if (value >> 4)
					{
						...
						core_memcpy(opll_sound.ch[ch].tone, oplltone[value >> 4], 8);
					...
					else if ((opll_sound.ch[ch].toneno != 0x10) || (opll_sound.toneupdate & (1 << ch)))
					{
						...
						core_memcpy(opll_sound.ch[ch].tone, opll_sound.usertone, 8);
						opll_sound.ch[ch].update |= OPLL_UPDATE_TONE;
					}
				}
				opll_sound.ch[ch].op[1].tl = (value & 0xF) << 2;
...
static void OPLLSoundWrite(u32 address, u8 value)
{
	if(address == 0x9010)
	{
...
//	double a;
	u32 cps;
	core_memset(&opll_sound, 0, sizeof(OPLLSOUND));
...
*/
	...
	opll_sound.lfo[0].spd = DivFix(NES_BASECYCLES, (u32)((6 * 72 * (1 << AM_BITS)) * 3.7), 18);
