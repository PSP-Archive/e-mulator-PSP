...
void NES_APU_Init()
{
    ...
    sample_rate = 44100; //Snd_GetSampleRate();
    sample_bits = 16;    //Snd_GetSampleBits();
...
{
    nes_apu_reset();
    core_memset(g_APU.regs, 0x00, sizeof(g_APU.regs));
}
...
{
    if( HAL_Sound() ) {
        nes_apu_process(halSnd.R32,length);
        HAL_Sound_Proc32m(halSnd.R32,length);
    }
}
...
void NES_APU_load_regs(const u8 new_regs[0x16])
{
    ...
    for(i = 0; i < 0x16; i++) {
        ...
        if((i >= 0x10) && (i <= 0x13)) {
            ...
            nes_apu_write(0x4000 + i, new_regs[i]);
            nes_apu_write_cur(0x4000 + i, new_regs[i]);
        }
    }
}
...
void NES_APU_get_regs(u8 reg_array[0x16])
{
 // copy last written values
    core_memcpy(reg_array, g_APU.regs, 0x16);
