...
#define V(x) ((x) << 19)
static const u32 mmc5_spd_limit[8] =
{
...
static s32 MMC5SoundSquareRender(MMC5_SQUARE *ch)
{
	u32 output;
	if (ch->update)
	{
...
{
	core_memset(ch, 0, sizeof(MMC5_SQUARE));
	ch->freq = SAMPLE_RATE;
	ch->cps = DivFix(NES_BASECYCLES, 12 * ch->freq, 19);
}

static s32 MMC5SoundRender(void)
{
	s32 accum = 0;
...
static void MMC5SoundVolume(u32 volume)
{
...
static void MMC5SoundWrite(u32 address, u8 value)
{
	if (0x5000 <= address && address <= 0x5015)
	{
		switch (address)
		{
			...
			case 0x5004: case 0x5006: case 0x5007:
				...
				break;
			case 0x5011:
				mmc5_sound.da.output = ((s32)value) - 0x80;
				break;
			case 0x5010:
...
{
	core_memset(ch, 0, sizeof(MMC5_DA));
}
