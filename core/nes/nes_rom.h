...
typedef struct _NES_ROM
{
	...
	u8 trainer[TRAINER_LEN];
	u8 ROM_banks[MAX_ROM_BANK];
	u8 VROM_banks[MAX_VROM_BANK];
...
u8 NES_ROM_get_mapper_num();
u8 NES_ROM_get_unifmapper_num();

u8  NES_ROM_has_save_RAM();
u8  NES_ROM_has_trainer();
u8  NES_ROM_is_VSUnisystem();

u8 NES_ROM_get_num_16k_ROM_banks();
u8 NES_ROM_get_num_8k_VROM_banks();

u8* NES_ROM_get_trainer();
u8* NES_ROM_get_ROM_banks();
u8* NES_ROM_get_VROM_banks();

//const char* NES_ROM_GetRomName();
//const char* NES_ROM_GetRomPath();

u8 NES_ROM_get_screen_mode();
void NES_ROM_GetROMInfoStr(char *);
char *NES_ROM_get_GameTitleName();

u32 NES_ROM_crc32();
u32 NES_ROM_crc32_all();
u32 NES_ROM_fds_id();

// for Best Play - Pro Yakyuu Special
u32 NES_ROM_get_size_SaveRAM();

u32 NES_ROM_get_UNIF_psize_16k();
u32 NES_ROM_get_UNIF_csize_8k();
